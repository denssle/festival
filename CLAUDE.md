...
- **CI/CD:** Zwei GitHub-Action-Workflows: `.github/workflows/tests.yml` führt Unit-Tests, Playwright-Tests und den Smoke-Test bei jedem Push auf `main` oder Pull Requests aus; `.github/workflows/deploy.yml` deployt nach erfolgreichem Test-Workflow auf `main` (via `workflow_run`).
  ...
  - **`tests.yml` hat drei Jobs:** `unit` fährt die Vitest-Suite (`npm run test:unit -- --run`, seit 2026-09; vorher liefen die Unit-Tests nur lokal). `test` fährt die Playwright-Suite über `PLAYWRIGHT=true` gegen In-Memory-SQLite (kein MariaDB nötig). `smoke` ist der **Produktions-Smoke-Test** (seit v0.7.29): Er baut das Artefakt, installiert wie der Host nur `npm ci --omit=dev` und startet es per `npm run start-server` gegen einen **MariaDB-Service-Container** (`scripts/smoke-test.sh`). Geprüft werden zwei Szenarien: frische DB (Schema aus Migrationen) und Bestands-DB ohne `SequelizeMeta` (Baseline-Stempel). Der Test verifiziert über `/api/health` explizit `dialect: 'mariadb'` — sonst würde ein stiller Rückfall auf SQLite den Test fälschlich grün machen.
