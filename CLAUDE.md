...
- **CI/CD:** Zwei GitHub-Action-Workflows: `.github/workflows/tests.yml` führt Unit-Tests, Playwright-Tests und den Smoke-Test bei jedem Push auf `main` oder Pull Requests aus; `.github/workflows/deploy.yml` deployt nach erfolgreichem Test-Workflow auf `main` (via `workflow_run`).
  - Deployment erfolgt automatisch per `rsync` auf Uberspace nach erfolgreichen Tests auf dem `main` Branch.
  - **WICHTIG:** Produktion läuft über `@sveltejs/adapter-node`. Die Pipeline baut mit `vite build` einen eigenständigen Node-Server unter `build/`; der Supervisor-Service startet ihn über `npm run start-server` (= `PORT=5173 BODY_SIZE_LIMIT=2M node --env-file=.env build`) im Verzeichnis `~/festival-app`. Die Pipeline installiert vor dem Restart die Produktions-Abhängigkeiten auf dem Host (`npm ci --omit=dev`). Kein `vite dev`/`npm install` mehr bei jedem Restart → Start in Sekunden.
  - **`tests.yml` hat drei Jobs:** `unit` fährt die Vitest-Suite (`npm run test:unit -- --run`, seit 2026-09; vorher liefen die Unit-Tests nur lokal). `test` fährt die Playwright-Suite über `PLAYWRIGHT=true` gegen In-Memory-SQLite (kein MariaDB nötig). `smoke` ist der **Produktions-Smoke-Test** (seit v0.7.29): Er baut das Artefakt, installiert wie der Host nur `npm ci --omit=dev` und startet es per `npm run start-server` gegen einen **MariaDB-Service-Container** (`scripts/smoke-test.sh`). Geprüft werden drei Szenarien: frische DB (Schema aus Migrationen), Bestands-DB ohne `SequelizeMeta` (Baseline-Stempel) und echte Abläufe per `curl` (seit v0.7.58: Registrierung, Festival, 1000-Zeichen-Kommentar, 422 bei zu langem Namen, Avatar-Upload mit ~1,4 MB gegen `BODY_SIZE_LIMIT`). Die Aufrufe schicken `Accept: text/html` – ohne den antwortet SvelteKit auf Form-Actions mit JSON und 200 statt mit Redirect oder Fehlerstatus. Der Test verifiziert über `/api/health` explizit `dialect: 'mariadb'` — sonst würde ein stiller Rückfall auf SQLite den Test fälschlich grün machen.
  - **Warum beides nötig ist:** Die SQLite-Suite fasst weder `node build` noch den MariaDB-Zweig von `startDB()` an. Genau in dieser Lücke entstanden drei Produktionsausfälle in Folge (v0.7.25–v0.7.28). Da `deploy.yml` per `workflow_run` am Gesamtergebnis von `tests.yml` hängt, blockiert ein fehlgeschlagener Smoke-Test den Deploy – ebenso rote Unit- oder Playwright-Tests. **Den Workflow-Namen `Run Playwright Tests` nicht ändern:** `deploy.yml` lauscht auf genau diesen Namen, ein umbenannter Workflow würde nie wieder einen Deploy auslösen.
...
- **Quadrat Rule (früher Triad):** Bei JEDER Änderung an Datenbankfeldern müssen IMMER diese vier Stellen aktualisiert werden:
  ...
  4. **Neue Migrationsdatei** in `src/lib/db/migrations/` (fortlaufend nummeriert, z. B. `0002-<beschreibung>.ts`, mit `up`/`down` auf `QueryInterface`). Die Baseline `0001-initial-schema.ts` ist EINGEFROREN und wird nie geändert. Der Drift-Test `migrations.spec.ts` schlägt fehl, wenn Modelle und Migrationen nicht mehr dasselbe Schema ergeben (Spalten, Unique-Indizes, Fremdschlüssel inkl. ON DELETE) – er ist die Absicherung dieser Regel.
     - **⚠️ Kein `changeColumn`/`removeColumn` unter SQLite:** Sequelize baut die Tabelle dort aus `describeTable()` neu und verliert dabei das `ON DELETE CASCADE` aller FKs; aus zusammengesetzten Unique-Indizes werden einzelne UNIQUE-Spalten. Spalten ändern: nach Dialekt verzweigen (Vorlage `0002-freitext-als-text.ts`) oder die Tabelle per `createTable` neu aufbauen (Vorlage `0003-…`). Spalten entfernen: rohes `ALTER TABLE … DROP COLUMN`, das können SQLite und MariaDB nativ (Vorlage `0004-…`). Migrationen **wiederholbar** bauen – DDL ist in MariaDB nicht transaktional; bricht ein Lauf mittendrin ab, läuft die Migration beim nächsten Start erneut über den halbfertigen Stand.
     - **⚠️ MariaDB-Eigenheiten, die SQLite nicht zeigt:** Ein CHECK darf keine Spalte verwenden, die ein FK per `ON UPDATE CASCADE` ändern kann (Fehler 1901) – solche FKs bekommen `ON UPDATE NO ACTION`. FK-Spalten brauchen exakt dieselbe Sortierung wie die Spalte, auf die sie zeigen (errno 150); Prod und CI stehen auf `utf8mb4_unicode_ci`.
...
- **Modelle & Beziehungen:**
  ...
  - **FestivalEvent:** Besitzt von Usern (via `UserId`), hat viele `GuestInformation`. Deren Antwort steht seit Migration 0004 (v0.7.63) in `answer`: `'yes' | 'maybe' | 'no'`, Typ `Answer` in `src/lib/models/Answer.ts`. Den soll auch die geplante Terminabstimmung verwenden. „Vielleicht“ und „nicht dabei“ werden mit Kommentar gegeben (gemeinsamer Handler `answerWithCommentHandler`), zählen nicht zur Gästezahl und erscheinen nicht unter „Festivals, zu denen ich gehe“.
  - **Comment:** Kommentare zu Festivals oder Nutzerprofilen, eigenes Datenbankmodell in `src/lib/db/model/comment.ts`. Das Ziel steht seit Migration 0003 (v0.7.60) in **genau einer** von zwei FK-Spalten: `FestivalEventId` oder `ProfileUserId`, beide mit `ON DELETE CASCADE`. Abgesichert wird das in MariaDB per CHECK-Constraint `comments_genau_ein_ziel`, in beiden Dialekten zusätzlich durch den Modell-Validator `exactlyOneTarget`. Im Code läuft das Ziel als `CommentTarget` (`{ festivalId }` | `{ profileUserId }`, `comment.service.ts`). Ein Kommentar an eine nicht existierende ID scheitert am FK; `saveComment` liefert dann `'Data Missing'`.
...
- **Initialisierung (seit v0.7.24 zweigleisig):** Erfolgt über `startDB()` in `src/lib/db/db.ts` und verzweigt nach Dialekt:
  - **Bestands-DBs ohne `SequelizeMeta`:** `stampBaselineIfLegacySchema()` (in `migrations.ts`, aufgerufen vor `up()`) trägt die Baseline als ausgeführt ein, **ohne sie auszuführen** – nötig für Datenbanken aus der `sync({ alter: true })`-Zeit, deren Schema bereits steht. Ohne den Stempel legt umzug das Schema erneut an und bricht beim `ADD UNIQUE INDEX` mit `Duplicate key name` (Fehler 1061) ab, da MariaDB dort kein `IF NOT EXISTS` kennt (genau dieser Fall in v0.7.28). Der Stempel greift eng: nur wenn noch keine Migration protokolliert ist UND die Tabelle `users` existiert.
...
- **Kontolöschung (seit v0.7.41):** `UserService.deleteAccount(userId)` löscht ein Konto samt allen daran hängenden Daten (Art. 17 DSGVO; UI unter `/settings`, Action `?/deleteAccount`, Passwortbestätigung + `QuestionDialog`). **Seit v0.7.60 erledigt das vollständig die FK-Kaskade**, der Service ruft nur noch `destroy()` auf – ein einziges DELETE, samt Kaskade atomar. Bis dahin stand das Kommentarziel in der polymorphen Spalte `comments.writtenTo` (Festival- ODER User-ID) ohne FK; Kommentare auf dem Profil und an den Festivals des Nutzers mussten von Hand abgeräumt werden, Letztere mit vorher eingesammelten Festival-IDs, weil die Festivals selbst per Kaskade an `FestivalEventService.deleteFestival` vorbei verschwinden. Migration 0003 hat die Spalte durch `FestivalEventId` und `ProfileUserId` ersetzt (siehe **Comment** oben). **Neue Beziehungen deshalb immer als FK mit `ON DELETE CASCADE` anlegen**, nie wieder als polymorphe Spalte. Abgesichert durch `account-deletion.spec.ts`.
...
- **Base-Pfad `/festival` (seit v0.7.36):** Die App wird unter `https://enzlor.uber.space/festival` ausgeliefert; die Domain-Wurzel gehört einem anderen Projekt. Gesetzt über `kit.paths.base` in `svelte.config.js`. Daraus folgt:
  ...
  - Der Session-Cookie liegt auf `SESSION_COOKIE_PATH` (= Base-Pfad, `src/lib/constants.ts`), damit er nicht an fremde Projekte derselben Domain geht. Beim Setzen und Löschen muss der Pfad identisch sein. Der Sprach-Cookie `lang` (`src/routes/language/+server.ts`) nutzt denselben Pfad.
  - **Harmlose Dev-Warnung:** `'lang' cookie does not exist for /festival/, but was previously set at /festival. Did you mean to set its 'path' to '/' instead?` taucht im Dev-Server (z. B. im Playwright-Log) auf und ist ein Fehlalarm. SvelteKit merkt sich nur im Dev-Modus prozessweit, unter welchem Pfad ein Cookie je gesetzt wurde, und warnt, sobald ein Request ohne den Cookie kommt und der gemerkte Pfad nicht exakt der URL entspricht – `/festival` ≠ `/festival/`. Der Cookie fehlt dann schlicht, weil dieser Browser-Kontext nie die Sprache gewechselt hat; der Browser schickt einen Cookie mit `Path=/festival` sehr wohl an `/festival/` und alles darunter. **Nicht „reparieren“:** `path: '/festival/'` schlösse die Adresse `/festival` aus, `path: '/'` schickte den Cookie an die fremden Projekte der Domain.
...
- **⚠️ `.env` muss explizit geladen werden:** `node build` liest – anders als `vite dev` – **keine** `.env`. `$env/dynamic/private` greift zur Laufzeit nur auf `process.env` zu. Deshalb gibt `start-server` die Datei per `--env-file=.env` mit (relativ zum Arbeitsverzeichnis `~/festival-app`). Fehlt die Datei ganz, bricht Node schon vor dem ersten Modul mit `.env: not found` (Exit 9) ab – `deploy.yml` prüft sie deshalb vor dem Restart. Ist sie da, aber leer, ist `MARIA_DB_USER` leer und MariaDB antwortet mit dem irreführenden `Access denied for user ''@… (using password: NO)` (Fehler 1045); `assertDatabaseCredentials()` (aufgerufen in `startDB()`) fängt das seit v0.7.25 mit einer sprechenden Meldung ab. **Der Check darf nicht auf Modulebene stehen:** `vite build` importiert die Servermodule in seiner Analyse-Phase, ein Fehler beim Import bricht damit schon den Build ab – auch in der CI, die diese Variablen gar nicht kennt (passiert in v0.7.26).
...
- **Ablauf:** `deploy.yml` baut `build/` mit `npm run build`, rsynct nach `~/festival-app` (ohne `.env` und `node_modules`), prüft dort die Existenz der `.env`, installiert Prod-Deps via `npm ci --omit=dev` und startet den Supervisor-Service neu.
- **Supervisor-Service:** heißt `festival`, führt `npm run start-server` im Verzeichnis `~/festival-app` aus (Config unter `~/etc/services.d/festival.ini`). Restart via `supervisorctl restart festival`; danach Readiness-Check gegen `http://localhost:5173/festival/api/health`.
- **⚠️ Nicht in den Docroot deployen (seit v0.7.38):** `~/html` ist der Apache-Docroot und gehört dem `website`-Projekt, dessen rsync mit `--delete` **ohne excludes** läuft. Solange beide Projekte dorthin deployten, löschten sie sich gegenseitig; am 2026-08-11 nahm das die `.env` mit und legte die Produktion lahm (der `website`-Deploy räumte `~/html` leer, `festival` starb rund eine Stunde später beim ersten Request, der ein Asset von der Platte brauchte). Diese App wird von Node auf Port 5173 ausgeliefert und hat im Docroot nichts verloren – dort war zudem der komplette Quellbaum öffentlich lesbar (`/package.json`, `/src/…`, `/TODO.md`).
- **Blick in die Prod-Datenbank: `scripts/prod-db.sh`** (seit v0.7.42). `scripts/prod-db.sh` ohne Argument zeigt die Zeilenzahl je Tabelle, mit Query (`scripts/prod-db.sh "SELECT nickname FROM users"`) das Ergebnis als Tabelle oder mit `--json`. Ohne `--write` sind nur lesende Statements erlaubt. **Warum nicht einfach `mysql` auf dem Host:** Der Client kommt mit den Zugangsdaten aus `~/festival-app/.env` nicht durch (`Access denied`, Fehler 1045 – über Unix-Socket wie über TCP), obwohl die App sich mit denselben Werten verbindet. Ursache ist die Shell: `source` interpretiert den Wert (Expansion von `$`, Backslashes), `node --env-file` nimmt ihn literal. Das Skript geht deshalb denselben Weg wie die App – `node --env-file=.env` mit dem `mariadb`-Treiber aus `~/festival-app/node_modules`, per Pipe an `node --input-type=module` (eine Datei in `/tmp` fände das Modul nicht, da Node vom Dateipfad aus auflöst).
- **Backups (Stand 2026-09-25, geprüft auf dem Host):** Uberspace sichert von sich aus, ein eigener Cronjob ist nicht nötig.
  - **Datenbank:** täglich gegen 02:40 ein Dump aller DBs des Nutzers. Die Festival-DB heißt `enzlor_prod` (`MARIA_DB_USER` + `_` + `MARIA_DB_NAME`). Der jüngste Stand liegt unter `/mysql_backup/current/enzlor/enzlor_prod.sql.xz`, ältere unter `/mysql_backup/old/enzlor/enzlor_prod.<datum>.<nr>.sql.xz` (rund drei Wochen). Für den Nutzer lesbar. Zurückspielen – vorher unbedingt den Dienst stoppen: `supervisorctl stop festival && xz -dc <dump> | mysql enzlor_prod && supervisorctl start festival`.
  - **Home-Verzeichnis inkl. `~/festival-app/.env`:** `/backup/daily.1` … `daily.7` und `/backup/weekly.1` … `weekly.7` (rund sieben Wochen), Pfad darin `home/enzlor/festival-app/.env`. **Nicht** gesichert ist, was außerhalb von `/home` liegt – also auch nicht der Docroot `/var/www/virtual/<user>/html` hinter `~/html`. Deshalb war die `.env` am 2026-08-11 unwiederbringlich weg: Sie lag damals noch im Docroot.
  - **Grenze:** Alle Sicherungen liegen beim selben Anbieter. Eine Kopie außerhalb von Uberspace gibt es nicht.
...
- **Datenbank-Konsistenz:** ~~Langfristig auf echte Migrationen statt `sync({ alter: true })` umstellen.~~ _Erledigt (v0.7.24): umzug-Migrationen für MariaDB (siehe Abschnitt 2, „Initialisierung"). Den dialektspezifischen Drift-Schutz liefert seit v0.7.29 der Smoke-Test gegen MariaDB (Abschnitt 1)._
