...
- **CI/CD:** Zwei GitHub-Action-Workflows: `.github/workflows/tests.yml` führt die Playwright-Tests bei jedem Push auf `main` oder Pull Requests aus; `.github/workflows/deploy.yml` deployt nach erfolgreichem Test-Workflow auf `main` (via `workflow_run`).
  ...
  - **`tests.yml` hat zwei Jobs:** `test` fährt die Playwright-Suite über `PLAYWRIGHT=true` gegen In-Memory-SQLite (kein MariaDB nötig). `smoke` ist der **Produktions-Smoke-Test** (seit v0.7.29): Er baut das Artefakt, installiert wie der Host nur `npm ci --omit=dev` und startet es per `npm run start-server` gegen einen **MariaDB-Service-Container** (`scripts/smoke-test.sh`). Geprüft werden zwei Szenarien: frische DB (Schema aus Migrationen) und Bestands-DB ohne `SequelizeMeta` (Baseline-Stempel). Der Test verifiziert über `/api/health` explizit `dialect: 'mariadb'` — sonst würde ein stiller Rückfall auf SQLite den Test fälschlich grün machen.
  - **Warum beides nötig ist:** Die SQLite-Suite fasst weder `node build` noch den MariaDB-Zweig von `startDB()` an. Genau in dieser Lücke entstanden drei Produktionsausfälle in Folge (v0.7.25–v0.7.28). Da `deploy.yml` per `workflow_run` am Gesamtergebnis von `tests.yml` hängt, blockiert ein fehlgeschlagener Smoke-Test den Deploy.
