...
- **Kontolöschung (seit v0.7.41):** `UserService.deleteAccount(userId)` löscht ein Konto samt allen daran hängenden Daten in **einer Transaktion** (Art. 17 DSGVO; UI unter `/settings`, Action `?/deleteAccount`, Passwortbestätigung + `QuestionDialog`). Den Großteil erledigt die FK-Kaskade. **Nicht abgedeckt ist `comments.writtenTo`** – die Spalte ist polymorph (Festival- ODER User-ID) und hat deshalb bewusst keinen FK. Zwei Fälle sind daher explizit aufzuräumen: Kommentare **auf dem Profil** des Nutzers und Kommentare **an seinen Festivals**. Letzteres ist die subtile Stelle: Die Festivals verschwinden per Kaskade und damit **an `FestivalEventService.deleteFestival` vorbei**, das diesen Fall sonst abräumt – die Festival-IDs müssen deshalb **vor** dem Löschen eingesammelt werden, danach sind sie nicht mehr ermittelbar. Wer eine neue Beziehung auf `writtenTo` baut, muss sie hier mitziehen; abgesichert ist das durch `account-deletion.spec.ts`.
