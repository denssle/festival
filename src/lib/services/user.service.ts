...
export class UserService {
	 ...
	 *
	 * Alles erledigt die FK-Kaskade (siehe db.ts und die Migrationen): Profilbild, Session,
	 * eigene Festivals, eigene Gruppen, Zu-/Absagen, Gruppenmitgliedschaften,
	 * Freundschaften, Freundschaftsanfragen, alle vom Nutzer VERFASSTEN Kommentare
	 * (`writtenBy`) sowie – seit Migration 0003 – Kommentare auf SEINEM PROFIL
	 * (`ProfileUserId`) und an SEINEN FESTIVALS (`FestivalEventId`, zweistufig über das
	 * per Kaskade gelöschte Festival).
	 *
	 * Bis v0.7.59 mussten die beiden letzten Fälle hier von Hand abgeräumt werden, weil
	 * die Spalte `writtenTo` polymorph war und keinen FK haben konnte. Die Transaktion von
	 * damals entfällt mit: Übrig ist ein einziges DELETE, und das ist samt Kaskade atomar.
	 */
