...
- [ ] 🟡 **Gebautes Artefakt statt Dev-Server in Produktion:** Aktuell läuft produktiv `vite dev --host` (`start-server`), und `adapter-auto` findet kein Zielenvironment. Auf `@sveltejs/adapter-node` umstellen: `vite build` in der Pipeline, `node build` unter Supervisor starten (statt Dev-Server). Vorbild: Nachbarprojekt „drache" deployt ein gebautes `dist/` und startet kompiliert. _(Deployment)_
