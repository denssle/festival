...
- [ ] 🟢 **Suche – LIKE-Escaping:** `GroupService.searchGroups` setzt den Suchbegriff ungefiltert in `%${term}%` ein. Sequelize parametrisiert zwar (keine SQL-Injection), aber LIKE-Wildcards (`%`, `_`) aus der Eingabe werden nicht escaped – z. B. matcht `%` alles. Wildcards vor der Query maskieren. _(Gruppen)_
...
- [ ] 🟢 **`alert()` / `confirm()` ersetzen:** In `src/routes/festival/[festival_id]/+page.svelte` (Join-Fehlerpfad) wird `alert()` genutzt, in `src/routes/group/[group_id]/+page.svelte` (Löschen) native `confirm()`. Beides durch die vorhandenen `InfoDialog` / `QuestionDialog` ersetzen, konsistent zum Rest der App. _(UX)_
