...
- [ ] 🟡 **E2E-Test-Isolation/Flakiness:** Die Specs teilen sich eine In-Memory-SQLite-DB (ein Dev-Server). Isoliert laufen alle grün, im vollen Suite-Lauf schlagen unter Last vereinzelt Tests fehl (z. B. `festival.spec` „Festival bearbeiten", `settings.spec` „Profilbild hochladen": `waitForResponse`-Timeouts). Ursachen angehen: pro Spec sauber isolieren (statt globalem `/api/test/reset`-Truncate), Timeouts/Waits robuster machen, ggf. Retries lokal. _(Testen)_
